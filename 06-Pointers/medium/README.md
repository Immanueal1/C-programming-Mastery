# 🟡 Chapter 6: Medium Practice Problems

> Intermediate C programming challenges focusing on variable swapping via Call by Reference, Call by Value limitations, returning multiple values using output pointers, pointer-to-pointer (`int **pptr`) chains, midpoint calculations, safe division status returns, cascading pointer reassignments, pointer aliasing, and multi-result geometry functions.

---

## 📌 Short Description

This directory contains 10 intermediate practice problems designed to master Call by Reference variable swapping, multi-value return patterns via pointer parameters, double dereferencing (`**pptr`), status code/value separation, and pointer aliasing.

---

## 📚 Topics Covered

- [x] Swapping Two Variables via Pointers (`swap(&a, &b)`)
- [x] Demonstrating Call by Value Swapping Failures (`fakeSwap`)
- [x] Returning Multiple Values via Output Parameters (`calculate`)
- [x] Pointer to Pointer Declaration & Double Dereferencing (`int **pptr`, `**pptr`)
- [x] Multi-Value Output via Call by Reference (`getMidpoint`)
- [x] Base Variable Mutation through Double Pointers (`**pp = 50`)
- [x] Combining Return Status with Output Pointers (`safeDivide`)
- [x] Cascading Pointer Reassignments & Sequential Dereference Mutations
- [x] Pointer Aliasing (Multiple Pointers referencing the same memory address)
- [x] Multi-Result Geometry Functions (`calcRectangle`)
...
| :--- | :--- | :--- |
| [`11_swapping_two_variables_using_pointers.c`](./11_swapping_two_variables_using_pointers.c) | Swapping Two Variables Using Pointers | Call by Reference, Swapping, Temporary Variable |
| [`12_the_failed_swap_illusion.c`](./12_the_failed_swap_illusion.c) | The Failed Swap Illusion | Call by Value Limitation |
| [`13_returning_multiple_values.c`](./13_returning_multiple_values.c) | Returning Multiple Values | Pointer Parameters, Multiple Outputs |
| [`14_pointer_to_pointer.c`](./14_pointer_to_pointer.c) | Pointer to Pointer (**) | Pointer-to-Pointer Declaration, Double Dereferencing |
| [`15_the_midpoint_finder.c`](./15_the_midpoint_finder.c) | The Midpoint Finder | Multi-Value Output via Pointers |
| [`16_mutating_the_base_via_double_pointer.c`](./16_mutating_the_base_via_double_pointer.c) | Mutating the Base via ** | Double Dereferencing Mutation |
| [`17_safe_division_with_status.c`](./17_safe_division_with_status.c) | Safe Division with Status | Returning Status and Value via Pointers |
| [`18_cascading_updates.c`](./18_cascading_updates.c) | Cascading Updates | Pointer Chains |
| [`19_pointer_to_same_type_alias.c`](./19_pointer_to_same_type_alias.c) | Pointer to Same Type Alias | Value Tracing |
| [`20_area_and_perimeter_calculator.c`](./20_area_and_perimeter_calculator.c) | Area and Perimeter Calculator | Multiple Output Pointer Function |
