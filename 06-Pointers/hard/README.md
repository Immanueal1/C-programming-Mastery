# 🔴 Chapter 6: Hard Practice Problems

> Advanced C programming challenges focusing on triple-level pointer indirection (`int ***p3`), dangling local pointer swapping, multi-variable array-less in-place sorting, 5-parameter math operation dispatchers, format specifier vs. raw RAM memory mix-ups, double pointer reassignments (`int **ptrRef`), 3-way cascade rotations, time metric extractions, safe NULL pointer checks, and operator precedence (`(*a)++`).

---

## 📌 Short Description

This directory contains 10 advanced practice problems designed to master multi-level indirection, local pointer parameter swap traps, in-place pointer-based sorting, multi-result output dispatching, double pointer reassignments, safe `NULL` checks, and operator precedence resolution.

---

## 📚 Topics Covered

- [x] Multi-Level Pointer Indirection (`int ***p3`, `***p3 += 1`)
- [x] Local Pointer Address Swapping Failures (`swapPointers`)
- [x] In-Place Multi-Variable Sorting via Call by Reference (`sortThree`)
- [x] Multi-Output Dispatching with Status Parameters (`compute`)
- [x] Debugging Specifier Type Mismatches (`%d` for `&x` and `%p` for `x`)
- [x] Permanently Modifying Caller Pointers via Double Pointers (`changePointer`)
- [x] Three-Way Variable Cascade Rotations (`rotateSwap`)
- [x] Extracting Time Metrics into Multiple Output Pointers (`convertSeconds`)
- [x] Enterprise Safeguards: NULL Pointer Checks (`if (p == NULL)`)
- [x] Pointer Operator Precedence (`(*a)++` vs `*a++`)
...
| :--- | :--- | :--- |
| [`21_multi_level_pointer_tracing.c`](./21_multi_level_pointer_tracing.c) | Multi-Level Pointer Tracing | Pointer-to-Pointer Chain Tracing |
| [`22_the_dangling_swap_trick.c`](./22_the_dangling_swap_trick.c) | The Dangling Swap Trick | Swapping Local Pointers |
| [`23_sort_three_variables.c`](./23_sort_three_variables.c) | Sort Three Variables | Call by Reference, Multiple Swaps |
| [`24_math_operations_dispatcher.c`](./24_math_operations_dispatcher.c) | Math Operations Dispatcher | Returning multiple conditional values |
| [`25_the_address_vs_value_mixup.c`](./25_the_address_vs_value_mixup.c) | The Address vs Value Mix-up (Debugging Concept) | Strict Type Matching |
| [`26_the_double_pointer_reassignment_function.c`](./26_the_double_pointer_reassignment_function.c) | The ** Reassignment Function | Modifying a Pointer inside a Function |
| [`27_cascade_swapping.c`](./27_cascade_swapping.c) | Cascade Swapping | Three-way swapping via pointers |
| [`28_extracting_time_metrics.c`](./28_extracting_time_metrics.c) | Extracting Time Metrics | Pointers as Multi-Returns |
| [`29_null_pointer_concept.c`](./29_null_pointer_concept.c) | Null Pointer Concept | NULL checking |
| [`30_simulating_references_with_pointers.c`](./30_simulating_references_with_pointers.c) | Simulating References with Pointers | Pointers mimicking C++ References |
