# 🟡 Chapter 3: Medium Practice Problems

> Intermediate C programming challenges focusing on multi-condition logic (`&&`, `||`), leap year evaluation rules, cascading `else-if` grade ladders, nested triangle validations, and `switch-case` calculator menus.
...
| [`13_leap_year_logic.c`](./13_leap_year_logic.c) | Leap Year Logic | Nested `if` OR Complex Logical Operators |
| [`14_academic_tutors_grader.c`](./14_academic_tutors_grader.c) | Academic Tutor's Grader | `else-if` Ladder |
| [`15_the_switch_calculator.c`](./15_the_switch_calculator.c) | The Switch Calculator | `switch`, `char` input, `break` |
| [`16_powerbuilding_bmi_category.c`](./16_powerbuilding_bmi_category.c) | Powerbuilding BMI Category | Float comparison, `else-if` |
| [`17_character_classifier.c`](./17_character_classifier.c) | Character Classifier | ASCII Ranges, `if-else` |
| [`18_triangle_validator.c`](./18_triangle_validator.c) | Triangle Validator | Logical Logic, Nested `if` |
| [`19_month_days_finder.c`](./19_month_days_finder.c) | Month Days Finder | `switch` Fall-through |
| [`20_cartesian_quadrant_checker.c`](./20_cartesian_quadrant_checker.c) | Cartesian Quadrant Checker | `else-if`, AND operator |
