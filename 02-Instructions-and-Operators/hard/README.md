# 🔴 Chapter 2: Hard Practice Problems

> Advanced C programming challenges focusing on complex formula translation, bitwise operations, date extraction, math tricks, and hardware algorithms.

---

## 📌 Short Description

This folder contains 10 advanced practice problems designed to test low-level ADC voltage scaling, 3-digit number reversing, kinematic velocity solvers, packed date extraction, bitwise vs logical AND differences, triangle inequality logic, non-conditional rounding tricks, and bitwise XOR variable swapping.
...
- [x] ADC Voltage Mapping Formulas & Floating-Point Division Guards
- [x] 3-Digit Number Reversing via Chained Modulo `% 10` & Division `/ 10`
- [x] Kinematic Velocity Formula Calculation using `<math.h>` (`sqrt()`, `pow()`)
- [x] Packed 8-Digit Date Extraction (`DDMMYYYY` ➜ Day, Month, Year)
- [x] Pure Logical Expression Vowel Checking without `if`
- [x] Bitwise AND (`&`) vs Logical AND (`&&`) Truthiness Analysis
- [x] Triangle Inequality Theorem (`A+B>C && B+C>A && A+C>B`)
- [x] Non-Conditional Integer Rounding Up to Nearest 10 (`((num + 9) / 10) * 10`)
- [x] Sequential Household Electricity Bill & Subsidy Formulas
- [x] Hardware-Efficient Variable Swapping via Bitwise XOR (`A = A ^ B`)
...
| :--- | :--- | :--- |
| [`21_microplastic_sensor_adc_scaling.c`](./21_microplastic_sensor_adc_scaling.c) | Microplastic Sensor ADC Scaling | Float Math, Explicit Casting, Formula Translation |
| [`22_reverse_a_3_digit_number.c`](./22_reverse_a_3_digit_number.c) | Reverse a 3-Digit Number | Chained Modulo and Division |
| [`23_physics_kinematics_solver.c`](./23_physics_kinematics_solver.c) | Physics Kinematics Solver | Precedence, Arithmetic, `<math.h>` |
| [`24_day_month_year_extraction.c`](./24_day_month_year_extraction.c) | Day/Month/Year Extraction | Modulo/Division Logic |
| [`25_boolean_vowel_detector_lowercase.c`](./25_boolean_vowel_detector_lowercase.c) | Boolean Vowel Detector (Lowercase) | ASCII bounds, Logical Operators |
| [`26_bitwise_vs_logical_and_warning.c`](./26_bitwise_vs_logical_and_warning.c) | Bitwise vs Logical AND Warning | `&` vs `&&` |
| [`27_the_triangle_inequality_theorem.c`](./27_the_triangle_inequality_theorem.c) | The Triangle Inequality Theorem | Logical Expressions |
| [`28_rounding_up_to_the_nearest_multiple_of_10.c`](./28_rounding_up_to_the_nearest_multiple_of_10.c) | Rounding up to the Nearest Multiple of 10 | Advanced Integer Math |
| [`29_bhilai_power_grid_billing.c`](./29_bhilai_power_grid_billing.c) | Bhilai Power Grid Billing | Complex sequential expressions |
| [`30_the_no_3rd_variable_trick_with_xor.c`](./30_the_no_3rd_variable_trick_with_xor.c) | The "No 3rd Variable" Trick with XOR | Bitwise XOR (`^`), Assignment |
