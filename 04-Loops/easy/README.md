# 🟢 Chapter 4: Easy Practice Problems

> Beginner-friendly C programming challenges focusing on basic `for`, `while`, and `do-while` loops, character and float iterators, pre/post increment verification, and fundamental `break`/`continue` statements.

---

## 📌 Short Description

This directory contains 10 fundamental practice problems designed to build core confidence with loop constructs, iterator initialization, condition testing, pre/post operator rules, and basic iteration control statements.

---

## 📚 Topics Covered

- [x] Sequential Counting with `for` Loops
- [x] Countdown Timers with `while` Loops & Decrement Operators (`--`)
- [x] Custom Loop Step Increments (`i += 2`)
- [x] Guaranteed Execution with `do-while` Loops & Trailing Semicolons `;`
- [x] Character Iterators (`char ch = 'A'; ch <= 'Z'; ch++`)
- [x] Early Loop Termination with `break`
- [x] Iteration Skipping with `continue`
- [x] Post-Increment Evaluation Rules (`x++`)
- [x] Pre-Increment Evaluation Rules (`++y`)
- [x] Floating-Point Loop Stepping (`float f = 0.0; f <= 1.0; f += 0.2`)

---

## 🎯 Learning Objectives

- Construct single-loop structures using `for`, `while`, and `do-while`.
- Implement non-integer loop counters (`char`, `float`).
- Observe and verify the operational difference between pre-increment (`++i`) and post-increment (`i++`).
- Use `break` to exit loops early and `continue` to skip specific iterations.
...
| :--- | :--- | :--- |
| [`01_the_forward_counter.c`](./01_the_forward_counter.c) | The Forward Counter | `for` Loop, Integer Iterator |
| [`02_the_liftoff_countdown.c`](./02_the_liftoff_countdown.c) | The Liftoff Countdown | `while` Loop, Decrement Operator (`--`) |
| [`03_even_number_stepper.c`](./03_even_number_stepper.c) | Even Number Stepper | Custom Updation (`i += 2`) |
| [`04_the_do_while_guarantee.c`](./04_the_do_while_guarantee.c) | The Do-While Guarantee | `do-while` Loop, Semicolon Rule |
| [`05_the_alphabet_printer.c`](./05_the_alphabet_printer.c) | The Alphabet Printer | Character Iterators |
| [`06_the_emergency_stop.c`](./06_the_emergency_stop.c) | The Emergency Stop (break) | `break` Statement |
| [`07_the_number_skipper.c`](./07_the_number_skipper.c) | The Number Skipper (continue) | `continue` Statement |
| [`08_post_increment_verification.c`](./08_post_increment_verification.c) | Post-Increment Verification | Post-Increment (`i++`) |
| [`09_pre_increment_verification.c`](./09_pre_increment_verification.c) | Pre-Increment Verification | Pre-Increment (`++i`) |
| [`10_the_float_stepper.c`](./10_the_float_stepper.c) | The Float Stepper | Float Iterators |
