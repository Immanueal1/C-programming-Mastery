# 🟡 Chapter 10: Medium Practice Problems

> Intermediate C programming challenges focusing on sequential `fscanf()` loops, dynamic logger appenders (`"a"` mode), `while(1)` EOF traversal, character counters, multi-file stream copy utilities (`fgetc()` + `fputc()`), multiplication table generation via `fprintf()`, dynamic filename inputs, line counting via newline detection (`\n`), multi-phase read/append data merging, and whitespace literal reading with `fgetc()`.

---

## 📌 Short Description

This directory contains 10 intermediate practice problems designed to master sequential formatted reading with `fscanf()`, appending logs via `"a"` mode with newlines, traversing file streams character-by-character using infinite `while(1)` loops with `break`, implementing character and line counters, copying files across two `FILE *` streams, generating tabular data with `fprintf()`, opening files using dynamic user-entered strings, executing two-phase read/append data updates, and demonstrating literal character extraction using `fgetc()`.

---

## 📚 Topics Covered

- [x] Sequential Formatted File Input via `fscanf()`
- [x] Logging & Data Appending using `"a"` Mode (`fprintf(fptr, "%d\n", id)`)
- [x] Constructing Standard Infinite `while(1)` EOF Traversal Loops
- [x] Character Counting Algorithms using `fgetc()` & Counter Increment
- [x] Multi-Stream File Copying (`fgetc(src)` ──► `fputc(dest)`)
- [x] Generating Structured Reports & Tables using `fprintf()` Loops
- [x] Dynamic User Filename Input & Safety Guarding
- [x] Line Counting Algorithms via Newline (`\n`) Character Detection
- [x] Multi-Phase File Operations (Read ➜ Close ➜ Append ➜ Close)
- [x] Literal Whitespace & Control Character Reading via `fgetc()`
...
| :--- | :--- | :--- |
| [`11_multi_integer_reader.c`](./11_multi_integer_reader.c) | Multi-Integer Reader | `fscanf()` loop |
| [`12_the_appending_logger.c`](./12_the_appending_logger.c) | The Appending Logger | `"a"` mode, `fprintf()` |
| [`13_the_end_of_file_traversal.c`](./13_the_end_of_file_traversal.c) | The End Of File (EOF) Traversal | `fgetc()`, `while(1)`, `EOF` |
| [`14_character_counter.c`](./14_character_counter.c) | Character Counter | `fgetc()`, `EOF`, Counters |
| [`15_the_copycat.c`](./15_the_copycat.c) | The Copycat | Reading and Writing simultaneously, `fgetc()`, `fputc()` |
| [`16_multiplication_table_generator.c`](./16_multiplication_table_generator.c) | Multiplication Table Generator | `fprintf()`, Loops |
| [`17_safe_file_opener.c`](./17_safe_file_opener.c) | Safe File Opener | User-provided filename, `NULL` check |
| [`18_line_counter.c`](./18_line_counter.c) | Line Counter | `fgetc()`, `\n` detection |
| [`19_formatted_data_merger.c`](./19_formatted_data_merger.c) | Formatted Data Merger | Multiple `fscanf()`, `fprintf()` |
| [`20_the_invisible_space.c`](./20_the_invisible_space.c) | The Invisible Space | `fgetc()` vs `fscanf()` behavior |
