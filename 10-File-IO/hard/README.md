# 🔴 Chapter 10: Hard Practice Problems

> Advanced C programming challenges focusing on dual file stream management, character case transformation with ASCII arithmetic (`ch - 32`), sequential multi-file content merging, character frequency counting, proving `"w"` overwrite destruction, multi-session log rotation simulation (`"w"` ➜ `"a"` ➜ `"r"`), empty file detection (`count == 0`), selective character filtering (vowel extraction), parallel `fscanf()` + `fprintf()` data pipeline formatting, and complete persistent data lifecycle tracking (Keyboard ➜ RAM ➜ Disk ➜ RAM ➜ Screen).

---

## 📌 Short Description

This directory contains 10 advanced practice problems designed to master dual file pointer stream management, ASCII arithmetic transformation during file copying, sequential multi-file content concatenation into a single file, targeted character frequency searching, empirical verification of `"w"` mode file contents truncation, multi-phase log rotating simulations, empty file detection handling, vowel character extraction into filtered files, parallel formatted data reading and report generation, and tracing complete persistent data lifecycle pathways across RAM, Disk, and Console.

---

## 📚 Topics Covered

- [x] Dual File Stream Management (`fptr_src` and `fptr_dest`)
- [x] Case Conversion via ASCII Arithmetic (`ch - 32`) during Stream Copying
- [x] Sequential Multi-File Merging (`file1.txt` + `file2.txt` ──► `merged.txt`)
- [x] Character Frequency Search & Conditional Counting across File Streams
- [x] Empirical Proof of `"w"` Mode Overwrite Behavior
- [x] Log Rotating Simulation (`"w"` Initial Clear ➜ `"a"` Append Log ➜ `"r"` Read)
- [x] Handling Empty Files (`count == 0` check on first `fgetc()`)
- [x] Conditional Character Filtering (Vowel Extraction into target file)
- [x] Parallel `fscanf()` + `fprintf()` Data Report Pipelines
- [x] Persistent Data Lifecycle Ping-Pong (Keyboard ──► RAM ──► Disk ──► RAM ──► Screen)
...
| :--- | :--- | :--- |
| [`21_even_number_filter.c`](./21_even_number_filter.c) | Even Number Filter | Multiple Files, Formatted I/O, Logic |
| [`22_the_capitalizer.c`](./22_the_capitalizer.c) | The Capitalizer | `fgetc()`, ASCII Logic, `fputc()` |
| [`23_file_merge_utility.c`](./23_file_merge_utility.c) | File Merge Utility | Sequential File Reading |
| [`24_character_frequency_search.c`](./24_character_frequency_search.c) | Character Frequency Search | `fgetc()`, Conditional Counting |
| [`25_the_overwrite_proof.c`](./25_the_overwrite_proof.c) | The Overwrite Proof | `"w"` vs `"a"` mode mechanics |
| [`26_log_rotating_simulator.c`](./26_log_rotating_simulator.c) | Log Rotating Simulator | Mixed Modes, State Management |
| [`27_blank_file_handler.c`](./27_blank_file_handler.c) | Blank File Handler | EOF on first read |
| [`28_vowel_extractor.c`](./28_vowel_extractor.c) | Vowel Extractor | Logic with `fgetc()`, Multiple Files |
| [`29_the_data_formatter.c`](./29_the_data_formatter.c) | The Data Formatter | `fscanf()` and `fprintf()` parallel |
| [`30_file_io_ping_pong.c`](./30_file_io_ping_pong.c) | File I/O Ping-Pong | Memory and File Direction flow |
