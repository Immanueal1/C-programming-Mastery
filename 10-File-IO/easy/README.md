# 🟢 Chapter 10: Easy Practice Problems

> Beginner-friendly C programming challenges focusing on file pointers (`FILE *`), file modes (`"r"`, `"w"`, `"a"`, `"wb"`), `NULL` safety checking (`if (fptr == NULL)`), formatted file I/O (`fprintf()`, `fscanf()`), character-by-character file I/O (`fputc()`, `fgetc()`), and the mandatory **OPEN ➜ WORK ➜ CLOSE** workflow (`fclose()`).

---

## 📌 Short Description

This directory contains 10 fundamental practice problems designed to build core confidence with file pointer declarations (`FILE *fptr`), file creation and opening via `fopen()`, file access modes (`"r"`, `"w"`, `"a"`, `"wb"`), `NULL` pointer safety guards to prevent segmentation faults, formatted text output using `fprintf()`, formatted text input using `fscanf()`, single character writing using `fputc()`, single character reading using `fgetc()`, understanding the overwrite nature of `"w"` vs preservation in `"a"`, and closing file streams with `fclose()`.

---

## 📚 Topics Covered

- [x] File Pointer Declarations (`FILE *fptr`)
- [x] Opening Files with `fopen("filename", "mode")`
- [x] File Access Modes (`"r"`, `"w"`, `"a"`, `"wb"`)
- [x] Mandatory `NULL` Safety Checks (`if (fptr == NULL)`)
- [x] Formatted Output Writing to Files via `fprintf()`
- [x] Formatted Input Reading from Files via `fscanf()`
- [x] Appending Data to Existing Files (`"a"` mode)
- [x] Single Character File Dispatch via `fputc()`
- [x] Single Character File Retrieval via `fgetc()`
- [x] Proper File Closing Workflow via `fclose()`
...
| :--- | :--- | :--- |
| [`01_the_persistent_creator.c`](./01_the_persistent_creator.c) | The Persistent Creator | `FILE *`, `fopen()`, `"w"` mode, `fclose()` |
| [`02_the_missing_file_guard.c`](./02_the_missing_file_guard.c) | The Missing File Guard | `"r"` mode, `NULL` checking |
| [`03_the_formatted_writer.c`](./03_the_formatted_writer.c) | The Formatted Writer | `fprintf()`, Program ──► File |
| [`04_the_formatted_reader.c`](./04_the_formatted_reader.c) | The Formatted Reader | `fscanf()`, File ──► Program |
| [`05_the_appender.c`](./05_the_appender.c) | The Appender | `"a"` mode |
| [`06_single_character_dispatch.c`](./06_single_character_dispatch.c) | Single Character Dispatch | `fputc()`, Program ──► File |
| [`07_single_character_retrieval.c`](./07_single_character_retrieval.c) | Single Character Retrieval | `fgetc()`, File ──► Program |
| [`08_binary_mode_recognition.c`](./08_binary_mode_recognition.c) | Binary Mode Recognition | `"wb"` mode |
| [`09_the_overwrite_trap.c`](./09_the_overwrite_trap.c) | The Overwrite Trap | `"w"` mode behavior |
| [`10_the_safe_closer.c`](./10_the_safe_closer.c) | The Safe Closer | `fclose()`, Proper Workflow |
