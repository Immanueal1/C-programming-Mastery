# 🟡 Chapter 9: Medium Practice Problems

> Intermediate C programming challenges focusing on traversing arrays of structures, pass-by-value vs. pass-by-pointer function calls, inline `typedef struct` definitions, raw structure memory size calculations, string input into structure arrays, context-aware Dot (`.`) vs. Arrow (`->`) usage, `{0}` zero-fill effects on strings, direct structure-to-structure block copying (`r2 = r1`), and array of structures search logic.

---

## 📌 Short Description

This directory contains 10 intermediate practice problems designed to master Array of Structures iteration, pass-by-value function stack copying, pass-by-pointer in-place mutation using `ptr->member`, inline type aliasing via `typedef struct { ... } Alias;`, raw memory allocation calculations, string input into structure array elements, selection rules for Dot (`.`) vs Arrow (`->`), `{0}` zero-fill string behavior, and direct block assignment (`r2 = r1`).

---

## 📚 Topics Covered

- [x] Traversing Arrays of Structures using `for` loops & `arr[i].member`
- [x] Pass-by-Value Semantics & Local Stack Copy Behavior
- [x] Pass-by-Pointer Mutation using Address Passing (`&s1`) & Arrow Operator (`ptr->member`)
- [x] Inline `typedef struct { ... } Alias;` Definitions
- [x] Raw Structure Memory Allocation Calculations (Summing Member Byte Sizes)
- [x] Reading Console Strings into Array of Structures (`scanf("%s", arr[i].name)`)
- [x] Disambiguating Dot Operator (`.`) vs Arrow Operator (`->`) Context
- [x] Zero-Initialization (`{0}`) Effects on Integer, Float, and Character Array Members
- [x] Direct Structure-to-Structure Block Assignment (`r2 = r1`)
- [x] Linear Traversal & Maximum Value Search Algorithms across Arrays of Structures
...
| :--- | :--- | :--- |
| [`11_traversing_an_array_of_structures.c`](./11_traversing_an_array_of_structures.c) | Traversing an Array of Structures | Array of Structures, Loops, Dot Operator |
| [`12_pass_by_value_simulation.c`](./12_pass_by_value_simulation.c) | Pass By Value Simulation | Passing Structures to Functions, Pass-by-value |
| [`13_pass_by_address.c`](./13_pass_by_address.c) | Pass By Address (Modifying Originals) | Passing Structure Addresses, Arrow Operator |
| [`14_typedef_at_definition.c`](./14_typedef_at_definition.c) | Typedef at Definition | `typedef` with Structure Definition |
| [`15_the_memory_layout_estimator.c`](./15_the_memory_layout_estimator.c) | The Memory Layout Estimator | Structure Memory Layout |
| [`16_reading_strings_into_array_of_structures.c`](./16_reading_strings_into_array_of_structures.c) | Reading Strings into Array of Structures | Array of Structures, `scanf`, String Members |
| [`17_dot_vs_arrow_identifier.c`](./17_dot_vs_arrow_identifier.c) | Dot vs Arrow Identifier | Identifying `.` vs `->` context |
| [`18_zero_init_tracing.c`](./18_zero_init_tracing.c) | Zero Init Tracing | `{0}` Initialization, String Arrays |
| [`19_structure_assignment.c`](./19_structure_assignment.c) | Structure Assignment | Direct Structure Copying |
| [`20_finding_the_highest_cgpa.c`](./20_finding_the_highest_cgpa.c) | Finding the Highest CGPA | Array of Structures, Traversal logic |
